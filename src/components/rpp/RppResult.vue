...
import { CheckCircle2, PenTool, Download, Share2, Clock, BookOpen, Plus, FileText } from "lucide-vue-next";
...
const emit = defineEmits(["download", "create-new"]);
  ...
  <div class="space-y-10 max-w-7xl mx-auto pb-20">
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
      <div class="p-8 bg-white rounded-[2rem] border-2 border-blue-50 flex-grow shadow-sm relative overflow-hidden">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div class="space-y-2">
            <h1 class="text-3xl font-black text-slate-900">{{ resultData.title }}</h1>
            <p class="text-slate-500 max-w-xl">{{ resultData.subText }}</p>
          </div>
          <div class="flex gap-3">
            ...
            <button @click="emit('download')" class="px-5 py-3 bg-blue-600 text-white rounded-xl font-extrabold flex items-center gap-2 hover:shadow-xl hover:shadow-blue-200 transition-all"><Download class="w-4 h-4" /> Unduh PDF</button>
        ...
        <div class="relative flex items-center justify-center">
          <svg class="w-48 h-48 transform -rotate-90">
            ...
            <circle
              ...
              :stroke-dashoffset="2 * Math.PI * 80 * (1 - resultData.readability / 100)"
              stroke-linecap="round"
              class="text-blue-600"
            />
          </svg>
          <div class="absolute flex flex-col items-center">
            <span class="text-5xl font-black text-slate-900">{{ resultData.readability }}</span>
            <span class="text-xs font-bold text-slate-400">/100</span>
          </div>
        </div>
        <div :class="['px-6 py-2 rounded-full text-xs font-bold', getConfidenceColor(resultData.readability)]">{{ getConfidenceLabel(resultData.readability) }}</div>
      </div>
        ...
        <div class="relative flex items-center justify-center">
          <svg class="w-48 h-48 transform -rotate-90">
            ...
            <circle
              ...
              fill="transparent"
              :stroke-dasharray="2 * Math.PI * 80"
              :stroke-dashoffset="2 * Math.PI * 80 * (1 - resultData.accessibility / 100)"
              stroke-linecap="round"
              class="text-blue-600 shadow-xl"
            />
          </svg>
          <div class="absolute flex flex-col items-center">
            <span class="text-5xl font-black text-slate-900">{{ resultData.accessibility }}</span>
            <span class="text-xs font-bold text-slate-400">/100</span>
          </div>
        </div>
        <div :class="['px-6 py-2 rounded-full text-xs font-bold', getConfidenceColor(resultData.accessibility)]">{{ getConfidenceLabel(resultData.accessibility) }}</div>
      </div>
    </div>
    ...
    <div class="grid lg:grid-cols-2 gap-8">
      <div class="bg-white rounded-[2.5rem] p-10 border border-slate-200 space-y-8">
        ...
        <div class="space-y-8">
          <div v-for="item in resultData.strengths" :key="item.title" class="flex gap-4">
      ...
      <div class="bg-white rounded-[2.5rem] p-10 border border-slate-200 space-y-8">
        ...
        <div class="space-y-4">
          <div v-for="res in resultData.resources" :key="res.name" class="flex items-center gap-4 p-4 rounded-2xl border border-slate-50 hover:bg-slate-50 transition-all group cursor-pointer">
            <div class="w-14 h-14 bg-slate-900/5 rounded-xl flex items-center justify-center text-slate-600 group-hover:bg-blue-600 group-hover:text-white transition-all">
              <FileText v-if="res.icon === 'FileText'" class="w-6 h-6" />
              <component v-else :is="res.icon" class="w-6 h-6" />
            </div>
            <div class="flex-grow">
